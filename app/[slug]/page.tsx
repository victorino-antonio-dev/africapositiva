...
export default async function ContentPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="py-20"><div className="container-site">
      ...
      {page.kind === "channels" && <div className="grid gap-4 md:grid-cols-2">{officialChannels.map(c=><div key={c.name} className="flex items-center justify-between gap-5 rounded-2xl border border-stone-200 p-6"><div className="min-w-0"><p className="font-display font-semibold">{c.name}</p>{"url" in c ? <a href={c.url} target="_blank" rel="noreferrer" className="mt-1 block break-words text-sm text-orange-700 hover:underline">{c.value}</a> : <p className="mt-1 break-words text-sm text-stone-500">{c.value}</p>}</div><span className={`shrink-0 rounded-full px-3 py-1 text-xs font-bold ${c.verified ? "bg-green-100 text-green-800" : "bg-amber-100 text-amber-800"}`}>{c.verified ? "Confirmado" : "Por confirmar"}</span></div>)}<div className="mt-5 md:col-span-2"><SafetyNote /></div></div>}
